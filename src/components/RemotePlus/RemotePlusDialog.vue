...
export default {
  ...
  computed: {
    postDialog: {
      get () {
        return this.$store.state.plusRemoteDialog
      },
      set (val) {
        this.$store.commit('setPlusRemoteDialog', val)
      }
    }
  },
  methods: {
    handleCloseDialog () {
      ...
      this.$router.push('/remote-plus');
      this.$store.commit('setRemoteActivePlus', null)
    }
  },
  props: {
    article: {
