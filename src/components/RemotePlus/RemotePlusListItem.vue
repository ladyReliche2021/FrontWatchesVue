...
export default {
  ...
  methods: {
    viewPost () {
      this.$router.push('/plus/'+this.article.id)
      this.$store.commit('setRemoteActivePlus', this.article)
      this.$store.commit('setPlusRemoteDialog', true)
    },
    remove(item){
        this.$emit('delete',item);
    },
    edit(id){
        this.$router.push('/maintenance-remote-plus/'+id);
    }
  },
  created () {
